...
$$
我们也可以看作是将POLY2方法中$R^{n\times n}$的矩阵分解为两个矩阵$R^{n\times k}$和$R^{k\times n}$，其中第一个矩阵第i行与第二个矩阵第j列相乘即使特征i、j的特征交叉参数。

![image-20240430134835195](https://ayimd-pic.oss-cn-guangzhou.aliyuncs.com/image-20240430134835195.png)
